    ...
    a.prototype = {
        get: function() {
            ...
            if (this.hasher) {
                ...
                return this.murmurhash3_32_gc(a.join("###"), 31);
            }
        },
        murmurhash3_32_gc: function(a, b) {
            ...
            j = 0;
            while (j < d) {
                i = a.charCodeAt(j) & 255 | (a.charCodeAt(++j) & 255) << 8 | (a.charCodeAt(++j) & 255) << 16 | (a.charCodeAt(++j) & 255) << 24;
                ++j;
                i = (i & 65535) * g + (((i >>> 16) * g & 65535) << 16) & 4294967295;
                i = i << 15 | i >>> 17;
            ...
            switch (c) {
              case 3:
                i ^= (a.charCodeAt(j + 2) & 255) << 16;

              case 2:
                i ^= (a.charCodeAt(j + 1) & 255) << 8;

              case 1:
                i ^= a.charCodeAt(j) & 255;
                i = (i & 65535) * g + (((i >>> 16) * g & 65535) << 16) & 4294967295;
                i = i << 15 | i >>> 17;
                i = (i & 65535) * h + (((i >>> 16) * h & 65535) << 16) & 4294967295;
                e ^= i;
...
var AES = {
    ...
    cipher: function(a, b) {
        var c = 4;
        var d = b.length / c - 1;
        var e = [ new Array(c), new Array(c), new Array(c), new Array(c) ];
        var f = new Array(4 * c);
        var g, h;
        for (g = 0; g < a.length; g++) {
            e[g % 4][Math.floor(g / 4)] = a[g];
        }
        this.addRoundKey(e, b, 0, c);
        for (h = 1; h < d; h++) {
            this.subBytes(e, c, 0);
            this.shiftRows(e, c, 0);
            this.mixColumns(e, c, 0);
            this.addRoundKey(e, b, h, c);
        }
        this.subBytes(e, c, 0);
        this.shiftRows(e, c, 0);
        this.addRoundKey(e, b, d, c);
        for (g = 0; g < f.length; g++) {
            f[g] = e[g % 4][Math.floor(g / 4)];
        }
        return f;
    },
    Invcipher: function(a, b) {
        var c = 4;
        var d = b.length / c - 1;
        var e = [ new Array(c), new Array(c), new Array(c), new Array(c) ];
        var f = new Array(4 * c);
        ...
        this.addRoundKey(e, b, d, c);
        for (h = d - 1; h >= 1; h--) {
            this.shiftRows(e, c, 1);
            this.subBytes(e, c, 1);
            this.addRoundKey(e, b, h, c);
            this.mixColumns(e, c, 1);
        }
        this.shiftRows(e, c, 1);
        this.subBytes(e, c, 1);
        this.addRoundKey(e, b, h, c);
    ...
    subBytes: function(a, b, c) {
        var d, e;
        var c = c || 0;
        var f = c === 0 ? this.sbox : this.Invsbox;
        for (e = 0; e < b; e++) {
            for (d = 0; d < 4; d++) {
                a[d][e] = f[a[d][e]];
            }
    ...
    mixColumns: function(a, b, c) {
        var d, e, f;
        var g = new Array(b);
        var h = [ [ 2, 3, 1, 1 ], [ 14, 11, 13, 9 ] ];
        for (e = 0; e < b; e++) {
            for (d = 0; d < 4; d++) {
                g[d] = a[d][e];
            }
            for (d = 0; d < 4; d++) {
                a[d][e] = 0;
                for (f = 0; f < 4; f++) {
                    a[d][e] ^= this.FFmul(h[c][f], g[(d + f) % 4]);
                }
            }
        }
    },
    ...
    keyExpansion: function(a) {
        ...
        var c = 4;
        var d = b + 6;
        var e = new Array(c * (d + 1));
        var f = new Array(4);
        var g, h;
        for (g = 0; g < b; g++) {
            e[g] = [ a[4 * g], a[4 * g + 1], a[4 * g + 2], a[4 * g + 3] ];
        }
        for (g = b; g < e.length; g++) {
            e[g] = new Array(4);
            for (h = 0; h < 4; h++) {
                f[h] = e[g - 1][h];
            }
...
AES.Crypto = function(a) {
    this.String2Array = function(a, b) {
        var c;
        if (b != null) {
            var d = new Array(16);
            for (c = 0; c < 16; c++) {
                d[c] = a.charCodeAt(16 * b + c) || 0;
            }
        } else {
            var e = a.length <= 16 ? 16 : a.length <= 24 ? 24 : 32;
            var d = new Array(e);
            for (c = 0; c < e; c++) {
                d[c] = a.charCodeAt(c) || 0;
            }
        }
        return d;
    };
    this.Byte2Char = function(a) {
        var b;
        var c = "";
        for (b = 0; b < a.length / 2; b++) {
            c += String.fromCharCode(parseInt(a[b * 2], 16) * 16 + parseInt(a[b * 2 + 1], 16));
        }
        return c;
    };
    this.encrypt = function(a) {
        ...
        var h, i;
        for (h = 0; h < c; h++) {
            e = AES.cipher(this.String2Array(a, h), this.keySchedule);
            g = h * b;
            for (i = 0; i < 16; i++) {
                d[g + i] = e[i] < 16 ? "0" + e[i].toString(16) : e[i].toString(16);
            }
        }
        return d.join("");
    };
    this.decrypt = function(a) {
        var c = Math.ceil(a.length / b * .5);
        var d = new Array();
        var e, f, g;
        var h, i;
        a = this.Byte2Char(a);
        for (h = 0; h < c; h++) {
            e = AES.Invcipher(this.String2Array(a, h), this.keySchedule);
            g = h * b;
            for (i = 0; i < 16 && e[i] != 0; i++) {
                d[g + i] = String.fromCharCode(e[i]);
            }
        }
        return d.join("");
    };
    var b = 16;
    this.key = this.String2Array(a, null);
    this.keySchedule = AES.keyExpansion(this.key);
};

var CryptoUtils = function(a) {
    var b = 0;
    var c = "";
    var d = 8;
    function e(a, b) {
        ...
        for (var k = 0; k < a.length; k += 16) {
            ...
            var o = e;
            var p = f;
            c = g(c, d, e, f, a[k + 0], 7, -680876936);
            f = g(f, c, d, e, a[k + 1], 12, -389564586);
            e = g(e, f, c, d, a[k + 2], 17, 606105819);
            d = g(d, e, f, c, a[k + 3], 22, -1044525330);
            c = g(c, d, e, f, a[k + 4], 7, -176418897);
            f = g(f, c, d, e, a[k + 5], 12, 1200080426);
            e = g(e, f, c, d, a[k + 6], 17, -1473231341);
            d = g(d, e, f, c, a[k + 7], 22, -45705983);
            c = g(c, d, e, f, a[k + 8], 7, 1770035416);
            f = g(f, c, d, e, a[k + 9], 12, -1958414417);
            e = g(e, f, c, d, a[k + 10], 17, -42063);
            d = g(d, e, f, c, a[k + 11], 22, -1990404162);
            c = g(c, d, e, f, a[k + 12], 7, 1804603682);
            f = g(f, c, d, e, a[k + 13], 12, -40341101);
            e = g(e, f, c, d, a[k + 14], 17, -1502002290);
            d = g(d, e, f, c, a[k + 15], 22, 1236535329);
            c = h(c, d, e, f, a[k + 1], 5, -165796510);
            f = h(f, c, d, e, a[k + 6], 9, -1069501632);
            e = h(e, f, c, d, a[k + 11], 14, 643717713);
            d = h(d, e, f, c, a[k + 0], 20, -373897302);
            c = h(c, d, e, f, a[k + 5], 5, -701558691);
            f = h(f, c, d, e, a[k + 10], 9, 38016083);
            e = h(e, f, c, d, a[k + 15], 14, -660478335);
            d = h(d, e, f, c, a[k + 4], 20, -405537848);
            c = h(c, d, e, f, a[k + 9], 5, 568446438);
            f = h(f, c, d, e, a[k + 14], 9, -1019803690);
            e = h(e, f, c, d, a[k + 3], 14, -187363961);
            d = h(d, e, f, c, a[k + 8], 20, 1163531501);
            c = h(c, d, e, f, a[k + 13], 5, -1444681467);
            f = h(f, c, d, e, a[k + 2], 9, -51403784);
            e = h(e, f, c, d, a[k + 7], 14, 1735328473);
            d = h(d, e, f, c, a[k + 12], 20, -1926607734);
            c = i(c, d, e, f, a[k + 5], 4, -378558);
            f = i(f, c, d, e, a[k + 8], 11, -2022574463);
            e = i(e, f, c, d, a[k + 11], 16, 1839030562);
            d = i(d, e, f, c, a[k + 14], 23, -35309556);
            c = i(c, d, e, f, a[k + 1], 4, -1530992060);
            f = i(f, c, d, e, a[k + 4], 11, 1272893353);
            e = i(e, f, c, d, a[k + 7], 16, -155497632);
            d = i(d, e, f, c, a[k + 10], 23, -1094730640);
            c = i(c, d, e, f, a[k + 13], 4, 681279174);
            f = i(f, c, d, e, a[k + 0], 11, -358537222);
            e = i(e, f, c, d, a[k + 3], 16, -722521979);
            d = i(d, e, f, c, a[k + 6], 23, 76029189);
            c = i(c, d, e, f, a[k + 9], 4, -640364487);
            f = i(f, c, d, e, a[k + 12], 11, -421815835);
            e = i(e, f, c, d, a[k + 15], 16, 530742520);
            d = i(d, e, f, c, a[k + 2], 23, -995338651);
            c = j(c, d, e, f, a[k + 0], 6, -198630844);
            f = j(f, c, d, e, a[k + 7], 10, 1126891415);
            e = j(e, f, c, d, a[k + 14], 15, -1416354905);
            d = j(d, e, f, c, a[k + 5], 21, -57434055);
            c = j(c, d, e, f, a[k + 12], 6, 1700485571);
            f = j(f, c, d, e, a[k + 3], 10, -1894986606);
            e = j(e, f, c, d, a[k + 10], 15, -1051523);
            d = j(d, e, f, c, a[k + 1], 21, -2054922799);
            c = j(c, d, e, f, a[k + 8], 6, 1873313359);
            f = j(f, c, d, e, a[k + 15], 10, -30611744);
            e = j(e, f, c, d, a[k + 6], 15, -1560198380);
            d = j(d, e, f, c, a[k + 13], 21, 1309151649);
            c = j(c, d, e, f, a[k + 4], 6, -145523070);
            f = j(f, c, d, e, a[k + 11], 10, -1120210379);
            e = j(e, f, c, d, a[k + 2], 15, 718787259);
            d = j(d, e, f, c, a[k + 9], 21, -343485551);
            c = l(c, m);
            d = l(d, n);
            e = l(e, o);
            f = l(f, p);
    ...
    function k(a, b) {
        var c = n(a);
        if (c.length > 16) c = e(c, a.length * d);
        var f = Array(16), g = Array(16);
        for (var h = 0; h < 16; h++) {
            f[h] = c[h] ^ 909522486;
            g[h] = c[h] ^ 1549556828;
        }
        var i = e(f.concat(n(b)), 512 + b.length * d);
        return e(g.concat(i), 512 + 128);
    }
    function l(a, b) {
    ...
    function n(a) {
        var b = Array();
        var c = (1 << d) - 1;
        for (var e = 0; e < a.length * d; e += d) b[e >> 5] |= (a.charCodeAt(e / d) & c) << e % 32;
        return b;
    }
    function o(a) {
        ...
        for (var e = 0; e < a.length * 4; e++) {
            d += c.charAt(a[e >> 2] >> e % 4 * 8 + 4 & 15) + c.charAt(a[e >> 2] >> e % 4 * 8 & 15);
        }
        return d;
    }
    function p(a) {
        ...
        for (var e = 0; e < a.length * 4; e += 3) {
            var f = (a[e >> 2] >> 8 * (e % 4) & 255) << 16 | (a[e + 1 >> 2] >> 8 * ((e + 1) % 4) & 255) << 8 | a[e + 2 >> 2] >> 8 * ((e + 2) % 4) & 255;
            for (var g = 0; g < 4; g++) {
                if (e * 8 + g * 6 > a.length * 32) d += c; else d += b.charAt(f >> 6 * (3 - g) & 63);
            }
        }
        return d;
    }
    return {
        hex_md5: function(a) {
            return o(e(n(a), a.length * d));
        },
        b64_md5: function(a) {
            return p(e(n(a), a.length * d));
        },
        hex_hmac_md5: function(a) {
            return o(k(key, data));
        },
        b64_hmac_md5: function(a) {
            return p(k(key, data));
        },
        calcMD5: function(a) {
            return o(e(n(a), a.length * d));
...
window.onload = function() {
    var a = new AES.Crypto("webgeek"), b = new Fingerprint().get();
    (function() {
        if (!$("body").hasClass("option")) return false;
        var a = c("valid-btn"), e = c("unlockkey");
        ...
        chrome.storage.local.get("fp", function(a) {
            if (b == a.fp) {
                $(".valid").remove();
    ...
    (function() {
        var a = window.location.origin, b = window.location.pathname;
        console.info("plugin runing..");
        var c = "http://ptr.chaoxing.com/multimedia/log";
        console.log(g());
    })();
    function c(a) {
    ...
    function e() {
        var a = new Date();
        var b = "" + a.getFullYear() + a.getMonth() + a.getDate() + a.getHours();
        return b;
    }
    function f(a) {
        a = a.split("+").join(" ");
    ...
    function g() {
        function a(a) {
            ...
            return b && typeof b != "undefined" ? true : false;
        }
        var b = function() {
            setTimeout(function() {
                if (a("jQuery")) {
    ...
    var h = function() {
        ...
        a.src = "http://demo.itbeihe.com/fe-questions/check-unloaded-function/init.js";
        b.appendChild(a);
    };
    h();
